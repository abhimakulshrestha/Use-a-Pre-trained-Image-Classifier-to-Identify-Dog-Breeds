...
#
# PROGRAMMER: Abhima Kulshrestha
# DATE CREATED:5/07/2023
# REVISED DATE:
...
def print_results(
    ...
    print(
        "Percentage of correctly classified not dogs is {}".format(
    ...
    for key in results_stats_dic:
        if key.startswith("p"):
            print(
                "All the percentages include: {},{}".format(key, results_stats_dic[key])
            )
    # prints out if print_incorrect_dogs = True
    if (
        print_incorrect_dogs
        and results_stats_dic["pct_correct_notdogs"]
    ...
    if (
        ...
        for key, labels in results_dic.items():
            if sum(labels[3:]) == 2 and labels[2] == 0:
                print(
                    "Real identity:{} and classifier labels:{}".format(
                        labels[0], labels[1]
                    )
                )
    None
